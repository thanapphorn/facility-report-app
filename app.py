...
# -----------------------
# Initialize session_state  ← FIX 1: เพิ่มการ initialize
# -----------------------
    ...
    if st.button("ตรวจสอบ"):
        ...
        for r in st.session_state.reports:
            if r["ID"] == track_id:
                ...
                # FIX 2: decode base64 ก่อนแสดงรูปภาพ
                if r["Image"]:
                    image_bytes = base64.b64decode(r["Image"])
                    st.image(image_bytes, caption=r["ImageName"], width=300)
                else:
                    st.info("ไม่มีรูปภาพ")
        if not found:
...
# -----------------------
    ...
    # -----------------------
    # Pagination  ← FIX 3: ป้องกัน division / edge case เมื่อไม่มีข้อมูล
    # -----------------------
    rows_per_page = 10
    ...
    if st.button("แสดงรายละเอียด"):
        for r in st.session_state.reports:
            if r["ID"] == view_id:
                ...
                # FIX 2: decode base64 ก่อนแสดงรูปภาพ (Admin view)
                if r["Image"]:
                    image_bytes = base64.b64decode(r["Image"])
                    st.image(image_bytes, caption=r["ImageName"], width=400)
                else:
                    st.info("ไม่มีรูปภาพ")
